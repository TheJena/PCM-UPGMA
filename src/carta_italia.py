...
df = pd.read_excel("datasets/99_dialects_lat_long_geolocation_clean.xlsx")#.drop(columns="Unnamed: 0")
df.columns = ['Language', 'Label', 'Glottocode', 'Iso 639-3 Code', 'Dialect Group', 'Location', 'Latitude', 'Longitude']
# df
...
geo_df = geo_df.assign(lat=lambda _df: _df["geometry"].apply(
        lambda pt: pt.y), long=lambda _df: _df["geometry"].apply(
        lambda pt: pt.x),
...
for i, cluster in enumerate(clusters):
    ...
    for lat, long in df.loc[df.Label.isin(cluster),["Latitude","Longitude"]].itertuples(index=False):
        geo_df.loc[geo_df.lat.eq(lat) & geo_df.long.eq(long), "marker"] = marker
        geo_df.loc[geo_df.lat.eq(lat) & geo_df.long.eq(long), "values"] = i/len(clusters)
...
ax = gpd.GeoSeries(italy.to_crs(epsg=4326)['geometry'].unary_union).boundary.plot(ax=ax, alpha=0.5, color="#0767ba",zorder=2, lw=0.5)
